...
main()
{
	long int ensemble=1000000, ssl=100000, sum=0, pos, posmax;
	float frp, lp; long int i, j, ss; int kounter;
	double rnd;
	...
	for (ss=10;ss<=ssl;ss*=10)
	{
		...
		for (lp=0;lp<1;lp+=0.1)
		{
			int tfrp=0;
			int posmaxe=0;
			int sum=0;
			int enp=0;
			//#pragma omp parallel for private(i, pos ,j , posmax, kounter, rnd, lp) reduction(+:sum,tfrp,posmaxe,enp)
