...
void generator (int j)		//this is the generator which takes in the different ids for the processes and generates the required Mersenne Twister State
{
    	...
    	/* This trys to find a small Mersenne Twister with period 2^521-1. */
    		mts[j] = get_mt_parameter_id_st(32,19937,(rand () % 200)*j,4172);
...
int main ()
{
	...
	long int ssl = 1000000000000, ss;
	MPI_Init (NULL,NULL);
	MPI_Comm_size ( MPI_COMM_WORLD, &p );
	MPI_Comm_rank ( MPI_COMM_WORLD, &id );
	n_parts = n/p;
	nn_parts = (n-n_parts*p);
	if ( id == 0 )
	...
	for ( ss = ssl ; ss <= ssl ; ss *= 10 )
	{
		...
		for (i=0;i<n_parts;i++)
		{
			...
			{
				if ((i != 0 ) && (i % 10 == 0))
				{printf("ID %d is at 250 now after %lfs!\n", id, MPI_Wtime() - wtime);}
			}
		}
