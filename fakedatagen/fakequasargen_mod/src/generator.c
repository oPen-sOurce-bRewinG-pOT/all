...
//also, OMP based jobs, done

#define tnum 50 //number of objects
#define N 732 //number of days
typedef struct
{
	double time;
	double T[tnum];
...
main()
{
	...
	//init
	list[0].time = date;
	#pragma omp parallel for //type 1
	for (k=0;k<tnum;k++)
	{
		id = omp_get_thread_num();
		
		list[0].base[k] = basegen(0);
		list[0].T[k] = tempgen(T_avg,5e6); //bb param
		
		list[0].b[k] = plawexp(0,1); //oldval = 0, condition = 1 //pl param exp
		double totflux_c = totflux(limlowf,limhighf,list[0].T[k],list[0].base[k]); //bb flux
		list[0].a[k] = totflux_c/powerlawint(limlowf,limhighf,list[0].b[k]); //pl param normalizer
		
		list[0].r[k] = rgen(list[0].base[k],list[0].T[k],list[0].a[k],list[0].b[k]);
		
		if (list[0].r[k] < 0)
		{printf("negative here! %d\n", k);}
		
		list[0].j[k] = jgen(list[0].base[k],list[0].T[k],list[0].a[k],list[0].b[k]);
	...
	/*
	#pragma omp parallel for //type 2
	for (k=0;k<tnum;k++)
	{
		id = omp_get_thread_num();
	...
	*/
	...
	for (i=1;i<N;i++) //evolution type 1
	{
		...
		for (k=0;k<tnum;k++)
		{
			...
			double totflux_c = totflux(limlowf,limhighf,list[i].T[k]); //bb flux
			list[i].a[k] = totflux_c/powerlawint(limlowf,limhighf,list[i].b[k]); //pl param normalizer
